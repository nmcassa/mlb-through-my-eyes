"""
espn_cfb.py
Thin wrapper around ESPN's public "hidden" API for college football. Same
endpoint family as espn_nfl.py, under "football/college-football" instead
of "football/nfl". Mirrors espn_nfl.py's shape closely (same box score
categories: passing/rushing/receiving only) rather than sharing code with
it, same reasoning as espn_cbb.py vs espn_nba.py — small enough differences
(team count, postseason meaning) that two parallel files stay clearer than
one shared abstraction.

Confidence note: see espn_nfl.py's module docstring — same caveat applies
here. Run `python3 espn_cfb.py diagnose <game_id>` against a real game ID
if parsing looks off.
"""
...
# FBS + FCS combined is well over 250 programs — the default /teams page
# size is much smaller than that.
_TEAMS_LIMIT = 500
...
def find_teams(query: str) -> list[dict]:
    """
    Look up college football teams by name/mascot/abbreviation fragment
    (e.g. 'Georgia', 'Bulldogs', 'UGA'). Returns a list of
...
def fetch_team_schedule(team_id: str, season: str, season_types: list[str] | None = None) -> list[dict]:
    """
    Fetch a team's game log for a season (the year it starts in — same
    convention as espn_nfl.py). season_types: '1' preseason, '2' regular
    season, '3' postseason (bowl games + CFP). Defaults to ['2'].
...
def fetch_games_by_date(date: str, team_id: str | None = None) -> list[dict]:
    """
    Fetch all college football games played on a single date (YYYY-MM-DD),
    optionally filtered to one team. Uses the scoreboard endpoint with the
    large groups/limit params (same pattern as espn_cbb.py) since the
    default page size truncates well before every FBS+FCS game is returned.

    Raises RuntimeError on API failure.
    """
    espn_date = date.replace("-", "")
    data = _get("scoreboard", dates=espn_date, groups=80, limit=400)
...
# ── Boxscore (identical categories/parsing to espn_nfl.py) ───────────────────
...
def fetch_boxscore_data(game_id: str) -> dict:
    """Same shape as espn_nfl.fetch_boxscore_data() — see its docstring."""
...
def diagnose(game_id: str):
    """Run directly: `python3 espn_cfb.py diagnose <game_id>` — see espn_nfl.py's version."""
    import json
    data = _get("summary", event=game_id)
    box = data.get("boxscore", {})
...
if __name__ == "__main__":
    if len(sys.argv) == 3 and sys.argv[1] == "diagnose":
        ...
        print("Usage: python3 espn_cfb.py diagnose <game_id>")
